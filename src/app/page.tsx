...
// 2. Наші деталі (по 1 шт. в "Двигун" і "Система живлення" для кожного трактора)
const PRODUCTS = [
  // МТЗ
...
export default function AgroStore() {
  ...
  // Новий стейт: запам'ятовуємо, який вузол зараз відкритий (якщо null - показуємо сітку)
  const [selectedNode, setSelectedNode] = useState<string | null>(null);

  // Коли клікаємо на марку зліва — обов'язково скидаємо відкритий вузол, щоб повернутися в корінь
  const handleBrandChange = (brand: typeof TRACTOR_BRANDS[0]) => {
    setSelectedBrand(brand);
    setSelectedNode(null);
  };

  // Фільтруємо товари тільки для обраної марки та обраного вузла
  ...
  return (
      <div className="max-w-7xl mx-auto px-4 py-8 flex flex-col md:flex-row gap-10">
        ...
        {/* ЛІВА ПАНЕЛЬ: Марки техніки */}
        <aside className="w-full md:w-64 shrink-0">
          <h2 className="text-xs font-bold uppercase tracking-widest text-gray-400 mb-5 px-2">
            Марки техніки
          </h2>
              ...
              return (
                  <button
                      ...
                      className={`w-full flex items-center justify-between px-4 py-4 rounded-lg border transition-all ${
                          isSelected
                              ? "bg-[#1c221f] text-white border-[#323b36]"
                              : "bg-[#141816] hover:bg-[#1c221f]/60 text-gray-300 border-[#242926]"
                      }`}
                  >
                <span className="font-black tracking-wide text-lg">
                  {brand.name} <span className="text-xs font-normal text-gray-600 ml-1">({brand.count})</span>
                </span>
                    {isSelected
                        ? <ChevronRight className="h-5 w-5 text-[#facc15]" />
        ...
        {/* ЦЕНТР: Динамічний контент (Сітка вузлів АБО Список товарів) */}
        <main className="flex-1">

          {/* ЯКЩО ВУЗОЛ НЕ ОБРАНО -> ПОКАЗУЄМО СІТКУ ВУЗЛІВ */}
          {!selectedNode ? (
              <>
              ...
              /* ЯКЩО ВУЗОЛ ОБРАНО -> ПОКАЗУЄМО ТОВАРИ */
              <>
                <div className="flex flex-col mb-8 pb-4 border-b border-[#242926]">
                  {/* Кнопка "Назад" */}
                  <button
                      onClick={() => setSelectedNode(null)}
                      className="flex items-center gap-2 text-sm text-gray-400 hover:text-[#facc15] transition-colors mb-4 w-fit"
                  >
                ...
                {currentProducts.length === 0 ? (
                    // Якщо в масиві немає товарів для цієї комбінації
                    <div className="text-center py-20 bg-[#141816] rounded-xl border border-dashed border-[#323b36] flex flex-col items-center justify-center">
                      ...
                      <p className="text-gray-500 text-sm">Спробуйте обрати інший вузол або скористайтеся пошуком.</p>
                      <button
                          onClick={() => setSelectedNode(null)}
                          className="mt-6 px-6 py-2 bg-[#1c221f] hover:bg-[#242926] border border-[#323b36] rounded text-white text-sm transition-colors"
                      >
